...
class SVGHelper:
    ...
    @staticmethod
    def StyleGroup(strokeColor: RGBA, strokeWidth: float, dash: tuple[float, ...] = (1, 0),
                   fillColor: RGBA = RGBA(0, 0, 0, 0)) -> SVGElement:
        dasharray = ', '.join(str(v) for v in dash)

        return SVGElement(SVGElementType.GROUP, stroke=f"{strokeColor.to_hex()}", strokewidth=strokeWidth,
                          strokeopacity=strokeColor.opacity,
                          fill=f"{fillColor.to_hex()}", fillopacity=fillColor.opacity, strokelinejoin="round",
    ...
    def Line(p0: ndarray, p1: ndarray, strokeColor: RGBA, strokeWidth: float) -> SVGElement:
        return SVGElement(SVGElementType.LINE, x1=p0[0], y1=p0[1], x2=p1[0], y2=p1[1],
                          stroke=f"{strokeColor.to_hex()}", strokewidth=strokeWidth, strokelinecap="round")
...
def CreatefontClass(name: str, size: float, fill: RGBA, sizeUnit: str = "pt", style: str = "italic",
                    font: str = "serif") -> SVGElement:
    fontstyle: str = f"font: {style} {size}{sizeUnit} {font}; fill: {fill.to_hex()};"
